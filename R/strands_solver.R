...
get.word <- function(x, mat) {
  x <- x[is.na(x)==FALSE]
  ints <- matrix(as.integer(unlist(strsplit(x, split = ","))), ncol = 2, byrow = TRUE)
  mat[ints]
}

get.words <- function(x, mat) {
  words <- apply(x, 1, get.word, mat)
...
#' Strands Solver
#'
#' @param mat Matrix of puzzle letters,
#' @param dictionary_freq A data frame, where the first column is the dictionary of words to use, and the second column is the frequency of the corresponding word.
#' @param max_words_checked Maximum number of found words to check when searching for puzzle solutions.
#' @param word_minimum_characters Minimum character length for words to be included in the solution.
#' @param include_all_words_with_at_least_nchars When searching for solutions, include all words that exceed this character count.
#' @param remove_crosses Do not include words that have letters which cross over each other.
#' @param return_all_words_only Only return the words found, not final solutions.
...
#'
#' \preformatted{mat <- matrix(data = c(
#'  "T", "E", "N", "H", "C", "N",
#'  "O", "O", "K", "A", "U", "C",
#'  "R", "A", "H", "O", "L", "R",
#'  "A", "N", "P", "T", "O", "P",
...
#'  ), nrow = 8, byrow = TRUE)}
#'
#' strands_solver(mat = mat, dictionary_freq = unigram_freq)
#' @export
strands_solver <- function(mat, dictionary_freq, max_words_checked = 1000, word_minimum_characters = 4, include_all_words_with_at_least_nchars = 6, remove_crosses = TRUE, return_all_words_only = FALSE, check_solutions = TRUE, verbose = TRUE) {
  ...
  index <- expand.grid(1:dim(mat)[1], 1:dim(mat)[2])
  ...
  for (letter in 1:length(index$row_col)) {
    ...
    while(nrow(check_list)>0) {
      ...
      for (i in 1:nrow(check_list)) {
        ...
        words <- get.words(data.frame(current_row, dims_to_check, row.names = 1:length(dims_to_check)), mat = mat)
  ...
  if(remove_crosses==TRUE) {
    ...
    row.names(final_list) <- 1:nrow(final_list)
  }

  if (return_all_words_only) {
    final_list$word <- get.words(final_list, mat = mat)
  ...
  mat_words <- mat_words[row.names(mat_words) %in% row.names(candidates),]
